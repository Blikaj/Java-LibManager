...
public class generator {
    ...
    // это генератор изначального архива книг, списка сотрудников и посетителей
    // ниже три функции, в которых вручную набиты значения для всех трёх
    // сохраняются с помощью потоков вывода объектов, каждый в свой файл так, что в файле хранится только один объект
    // класса. Это удобно для чтения объектов, единственное, что потокобезопасности достигнуть не получится, тк
    // операция чтения будет блокировать единственный объект в файле.
